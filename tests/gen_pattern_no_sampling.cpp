...
double STRAIGHT_SAMPLING_RESOLUTION = 0.02;
// double STRAIGHT_SAMPLING_RESOLUTION = 0.01;
// double CURVE_SAMPLING_RESOLUTION = 0.03;
double CURVE_SAMPLING_RESOLUTION = 0.02;
...
std::string OUTFILE_PREFIX = "../patterns/patterns_";
...
int main(int argc, char** argv)
{
    ...
    fprintf(outfile, "totalnumberofprimitives: %d\n", N_total);
    ...
    fclose(outfile);
    ++count;
    // printf("Generated single cell action | count: %d\n", count);

    /// Generate stright-line macro actions
    for (auto xsize = 1; xsize <= x_max; ++xsize) {
    for (auto ysize = 1; ysize <= y_max; ++ysize) {
        ...
        ComputeAndPrintStraightMacroAction(xsize, ysize, filepath);
        ++count;
        // printf("count: %d\n", count);
    }
    }

    /// Generate rectangular macro actions
    for (auto xsize = 2; xsize <= x_max; ++xsize) {
    for (auto ysize = 2; ysize <= y_max; ++ysize) {
    ...
    printf("Generated [%d x %d] patterns\n", x_max, y_max);
}